...
for t in t_array:
    ...
    if t == 0:
        ...
        X_wake[i_t-1] = transform_local_to_global(trailing_vortex,0,alpha,X0_dot*t,Z0_dot*t)[0]
        Z_wake[i_t-1] = transform_local_to_global(trailing_vortex,0,alpha,X0_dot*t,Z0_dot*t)[1]
        ...
        L = np.sum(delta_p*l*np.cos(alpha))
        cl_t[i_t] = L / (0.5 * rho * U_inf**2 * fltplt4.c)
        p_dist_t[i_t, :] = rho*U_inf*np.hstack((np.squeeze(gamma[:fltplt4.N]), Gamma_wake))/l
